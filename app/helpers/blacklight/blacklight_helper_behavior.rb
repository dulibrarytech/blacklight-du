...
module Blacklight::BlacklightHelperBehavior
  ...
  def render_image_preview *args
    ...
    if value != nil && value != ""
      ...
      link = link_to image_tag(value, class: "tn-image"), options[:url], :target => "_blank"
      link.html_safe
    else
  ...
  # DU: Set all values to html_safe, in order to render the value(s) as a link.
  def render_field_link_value value=nil, field_config=nil
    safe_values = Array(value).collect { |x| x.html_safe }
    safe_join(safe_values, (field_config.separator if field_config) || field_value_separator)
  end
